...
const getAvailableTimeSlots = async (
  ...
  console.log(`API FAKE: Buscando horários para ${date} e serviço ${serviceId}`);
...
const submitAppointment = async (
  ...
  try {
    ...
    const newAppointment = { ...formData, submittedAt: new Date().toISOString() };
...
const submitContactForm = async (
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Erro desconhecido';
    return { success: false, message: `Erro ao salvar no localStorage: ${message}` };
  }
};
